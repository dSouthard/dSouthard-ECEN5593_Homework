...
__global__ void histogram64Kernel(uint *d_Result, data_t *d_Data, uint dataCount, bool atomicAddFlag){
...
#pragma unroll
		...
		if (!atomicAddFlag)
			d_Result[blockIdx.x * HISTOGRAM64_BIN_COUNT + threadIdx.x] = sum;
		else{
			atomicAdd(d_Result + threadIdx.x, sum);
		}
	}
}
...
extern "C" void initHistogram64(bool atomicAddFlag){
	...
	if (!atomicAddFlag)
		// d_PartialHistograms is only allocated if not using atomic add
...
extern "C" void closeHistogram64(bool atomicAddFlag){
	if (!atomicAddFlag)
		// d_PartialHistograms was only allocated if not using atomic add
...
extern "C" void histogram64(
	...
	if (atomicAddFlag <= 0) {
		// Create a partial histogram, then merge the results
		...
		// Use atomic add, only need to call the first kernel
		histogram64Kernel<<<histogramCount, HISTOGRAM64_THREADBLOCK_SIZE>>>(
				d_Histogram,
				(data_t *)d_Data,
				byteCount / sizeof(data_t),
